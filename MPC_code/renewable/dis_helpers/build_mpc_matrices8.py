...
def build_mpc_matrices8(N, inv, dt, Wf, Wr, Wp, Pmax, Pmin, Pref):

    inv_map = {43:0,44:1,52:2,53:3,54:4,55:5,56:6,57:7}
    ...
    #Reduce to remove first voltage angle and keep only local Pc and xmpc
    E_p = 8*[0]
    E_p[u_key] = 1
    keep = np.array([0] + 16*[1] + E_p + E_p, dtype=bool)
    E = np.eye(len(keep))[np.flatnonzero(keep), :]

    Ac = E @ A_t @ E.T
    Bc = E @ B_t
    cp = 16*[0] + [1] + [0] 
    Cc = np.vstack([np.hstack([np.zeros((8,7)),np.eye(8),np.zeros((8,3))]), cp])
    Dc = np.zeros((9,1))

    sysc = ct.ss(Ac,Bc[:,u_key+1:u_key+2],Cc,Dc)
