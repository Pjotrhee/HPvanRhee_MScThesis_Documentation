...
from andes.models.renewable.sta_helpers.mpc_func import mpc_func
from andes.models.renewable.sta_helpers.build_mpc_matrices import build_mpc_matrices
from andes.models.renewable.sta_helpers.optimal_ref import optimal_ref
import numpy as np
...
class REGF1W_MPC_STAData(ModelData):
    """
    REGF1W_MPC_STA model data.
    """
    ...
    def __init__(self):
        ...
        self.N = NumParam(default=10, tex_name='N',
                             info='Centralized MPC prediction horizon',
                             non_negative=True,
                             )
        ...
        self.Wr = NumParam(default=3, tex_name='W_r',
...
class REGF1W_MPC_STAModel(Model):
    """
    ...
    def __init__(self, system, config):
        ...
        self.xehat = {}
        self.xnom = {}
        self.u_nom = np.array(8*[0.0])
        self.u_store = np.array(8*[0.0])
        self.f_store = {}
    ...
    def update_mpc(self, **kwargs):
        ...
        mpc_step_mult = int(self.mpc_step_mult.v[0])
        ...
        noise_std = float(self.noise_std.v[0])
        f_or_p = float(self.f_or_p.v[0])
        loss_prob = float(self.loss_prob.v[0])

        #Only build the matrices when this wasn't already done
        key = (N, dt_mpc, tuple(Wf), tuple(Wr), tuple(Wp), tuple(Pmax), tuple(Pref))
        if key not in self._mpc_cache:
            # build_mpc_matrices should accept N, inv, dt and return matrices
            mpc_data, ref_data = build_mpc_matrices(N, dt_mpc, Wf, Wr, Wp, Pmax, Pmin, Pref)
        ...
        if k_now == 0:
            self.u_nom = np.array(8*[0.0])
            self.u_store = np.array(8*[0.0])
            ### STEP 1 (Algorithm 5.7):
            self.xehat[k_now] = np.zeros((41,1)) # Extended, so could be called xe_hat or phi_hat
            self.xnom[k_now] = np.zeros((40,1))  
        ...
        if k_now % mpc_step_mult == 0 and k_now != 0: # Activate MPC only at MPC time step (not simulation time step)
            ...
            ### STEP 5 (Algorithm 5.7):
            # Read output measurements
            freq = (self.fn.v*self.f.v) - self.fn.v

            for i,f in enumerate(freq):
                if i in self.f_store and rng.random() < loss_prob:
                    freq[i] = self.f_store[i]
                else:
                    freq[i] = f
                self.f_store[i] = freq[i]


            rocof = dt_mpc*self.rocof.v*self.fn.v
            P_c = self.Pe.v - self.Pref.v
            # Add measurement noise
            noisy_freq = freq + rng.normal(0.0, noise_std, size=freq.shape)
            noisy_P_c = P_c + rng.normal(0.0, noise_std, size=P_c.shape)

            if f_or_p == 0.0:
                y_k = np.concatenate((noisy_freq,rocof,P_c))
            elif f_or_p == 1.0:
                y_k = np.concatenate((freq,rocof,noisy_P_c))
            ...
            if kkey not in self.xehat:
                u_k = self.u_store
                u_k = u_k.reshape(-1,1)
                xehat_next = mpc_data['A'] @ xehat_0 + mpc_data['B'] @ u_k + mpc_data['L'] @ (y_k.reshape(-1,1) - mpc_data['C'] @ xehat_0)
                self.xehat[kkey] = xehat_next

            ### STEP 6 (Algorithm 5.7):
            if kkey not in self.xnom:
                u_nom_k = self.u_nom
                u_nom_k = u_nom_k.reshape(-1,1)
                dhat_0 = ref_data['vd'] @ xehat_0
                xnom_next = ref_data['A'] @ xnom_0 + ref_data['Bd'] @ dhat_0 + ref_data['B'] @ u_nom_k
                self.xnom[kkey] = xnom_next

            ### STEP 2 (Algorithm 5.7):
            #Optimal set-point
            dhat_0 = ref_data['vd'] @ self.xehat[kkey]
            xhat_0 = ref_data['vx'] @ self.xehat[kkey]
            x_ref, u_ref = optimal_ref(ref_data, dhat_0)

            # Run the actual MPC optimization
            self.u_nom = mpc_func(xnom_0, dhat_0, x_ref, u_ref, N, mpc_data)
            u_correction = mpc_data['K'] @ (xhat_0 - xnom_0)
            self.u_store = self.u_nom + u_correction.T
            
        else: # Keep MPC u and xhat constant when not at MPC time step
            if k_now != 0:
                self.xehat[k_now] = self.xehat[k_now-1]
                self.xnom[k_now] = self.xnom[k_now-1]
            self.u_store = self.u_store

        u_mpc_list = self.u_store

        ### STEP 4 (Algorithm 5.7):
        return u_mpc_list
...
class REGF1W_MPC_STA(REGF1W_MPC_STAData, REGF1W_MPC_STAModel, REGF1W_MPC_STAPrimary,
    ...
    def __init__(self, system, config):
        REGF1W_MPC_STAData.__init__(self)

        REGF1W_MPC_STAModel.__init__(self, system, config)
        REGF1W_MPC_STAPrimary.__init__(self)
