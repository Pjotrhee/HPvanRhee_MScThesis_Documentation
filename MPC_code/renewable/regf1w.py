...
class REGF1WModel(Model):
    ...
    def __init__(self, system, config):
        ...
        #Recovery management
        self.rocof_flag = VarService(v_str='Indicator(abs(rocof)<(Rthr/fn))')

        self.latch1 = State(e_str= '(1-rocof_flag)*(1-latch2)*(-1/0.02)*(latch1 - 1) + latch2*(-1/0.02)*(latch1 - 0)')
        self.timer1 = State(e_str='latch1 + latch3*(-1/0.02)*(timer1 - 0)')
        self.time1_flag = VarService(v_str='Indicator(timer1>Tffr)')

        self.latch2 = State(e_str='time1_flag*rocof_flag*(1-latch3)*(-1/0.02)*(latch2 - 1) + latch3*(-1/0.02)*(latch2 - 0)')
        self.timer2 = State(e_str='latch2 + latch1*(-1/0.02)*(timer2 - 0)')
        self.time2_flag = VarService(v_str='Indicator(timer2>5*Trec)')
        
        self.latch3 = State(e_str='time2_flag*(1-latch1)*(-1/0.02)*(latch3 - 1) + latch1*(-1/0.02)*(latch3 - 0)')
        self.recov_flag = State(e_str='time1_flag*rocof_flag*(1 - time2_flag)*(-1/0.02)*(recov_flag - 1) + time2_flag*(-1/0.02)*(recov_flag - 0)')

        # s9 and s11
        self.PIerr = Algeb(e_str='PIerr - (-Psen_y + P_switch)', v_str='0') # For plotting

        #Switching Power reference, between recovery and normal/FFR mode
        self.P_pre = LagAntiWindup(u='Psen_y + Paux', K=1, T=self.Tpm, lower=self.Pmin, upper=self.Pmax)
        self.P_rec = State(e_str='(1-recov_flag)*(-(1/0.001)*(P_rec - P_switch)) + recov_flag*(-(1/Trec)*(P_rec - Pref))', v_str='Pref')        
        self.P_switch = Algeb(e_str='P_switch - ((1-recov_flag)*P_pre_y + recov_flag*P_rec)', v_str='Pref')

        self.Qsig = LagAntiWindup(u='Qsen_y + Qaux', K=1, T=self.Tpm,
                                  lower=self.Qmin, upper=self.Qmax,
                                  )

        self.PIplim = PIController(u='-Psen_y + P_switch',
...
class REGF1W(REGF1WData, REGF1WModel, REGF1WPrimary,
    ...
    def __init__(self, system, config):
        REGF1WData.__init__(self)

        REGF1WModel.__init__(self, system, config)
        REGF1WPrimary.__init__(self)
