...
from andes.models.renewable.cen_helpers.mpc_func import mpc_func
from andes.models.renewable.cen_helpers.build_mpc_matrices import build_mpc_matrices
from andes.models.renewable.cen_helpers.optimal_ref import optimal_ref
import numpy as np
...
class REGF1W_MPC_CENData(ModelData):
    """
    REGF1W_MPC_CEN model data.
    """
...
class REGF1W_MPC_CENModel(Model):
    """
    ...
    def update_mpc(self, **kwargs):
        ...
        if k_now % mpc_step_mult == 0 and k_now != 0: # Activate MPC only at MPC time step (not simulation time step)
            ...
            if kkey not in self.xhat:
                u_k = self.u_store
                u_k = u_k.reshape(-1,1)
                xhat_next = mpc_data['A'] @ xhat_0 + mpc_data['B'] @ u_k + mpc_data['L'] @ (y_k.reshape(-1,1) - mpc_data['C'] @ xhat_0)
            ...
            self.u_store = mpc_func(xhat_0, xe_ref, u_ref, N, mpc_data)
            
        else: # Keep MPC u and xhat constant when not at MPC time step
            if k_now != 0:
                self.xhat[k_now] = self.xhat[k_now-1]
...
class REGF1W_MPC_CEN(REGF1W_MPC_CENData, REGF1W_MPC_CENModel, REGF1W_MPC_CENPrimary,
    ...
    def __init__(self, system, config):
        REGF1W_MPC_CENData.__init__(self)

        REGF1W_MPC_CENModel.__init__(self, system, config)
        REGF1W_MPC_CENPrimary.__init__(self)
